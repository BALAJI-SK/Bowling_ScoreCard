const score=(rolls)=>{
if(!Array.isArray(rolls)){throw new Error('Unsportted Input')};
let frame=0;
let totalGamePoints=0;
for(;frame<10;frame++){
    let firstThrow=rolls.at(frame*2);
    let secondThrow=rolls.at(frame*2+1);
    let framePoints=firstThrow+secondThrow;
    if(firstThrow==10){
        framePoints=10+(secondThrow+rolls.at((frame+1)*2));
    }
    else if(framePoints==10){
        framePoints+=rolls.at((frame+1)*2);
    }
